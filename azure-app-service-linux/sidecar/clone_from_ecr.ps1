...
$artefacts=(
  [pscustomobject]@{srcPath='/dynatrace/dynatrace-codemodules';targetPath='/oneagent-codemodules';targetTagPostfix='amd64'}
)
...
foreach ($tech in $techs) {
  foreach($a in $artefacts) {
    ...
    $sourceRepo = "public.ecr.aws$($a.srcPath)"
    $targetRepo = "${targetRegistry}$($a.targetPath)"

    $sourceImage = "${sourceRepo}:${version}"
    #$sourceImage ="public.ecr.aws/dynatrace/dynatrace-codemodules:1.301.54.20241017-161011"
    ...
    iex "${containerRuntime} build --no-cache --build-arg=`"DT_BASEIMG=${sourceImage}`" -f Dockerfile.native -t ${targetRepo}:${versionTag}"
    
    #tag images using latest and rolling version
    iex "${containerRuntime} tag ${targetRepo}:${versionTag} ${targetRepo}:${rollingVersionTag}"
    iex "${containerRuntime} tag ${targetRepo}:${versionTag} ${targetRepo}:${lastestVersionTag}"
